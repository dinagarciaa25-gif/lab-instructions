...
**Answer:** The `datasaurus_dozen` data frame has 1,846 rows and 3 columns. The variables are `dataset`, `x`, and `y`.
...
**Answer:** The correlation coefficient between `x` and `y` for the dino dataset is approximately -0.0645, which indicates almost no linear relationship.
...
```{r star-plot, fig.fullwidth=TRUE}
...
- Compared to the dino dataset (r = -0.06447185), the star dataset's correlation is: **very similar. Both correlations are close to zero, even though the plots have very different shapes.**
...
```{r circle-plot, fig.fullwidth=TRUE}
ggplot(data = circle_data, mapping = aes(x = x, y = y)) +
  geom_point()
```

```{r circle-correlation}
circle_data %>%
  summarize(r = cor(x, y))
```
...
- Compared to the dino dataset, the circle dataset's correlation is: **very similar because both values are close to zero.**

- What do you notice about the correlation coefficients for dino, star, and circle? **All three correlation coefficients are very close to zero and very similar, even though their plots have completely different shapes. This shows why visualizing data is important.**
