...
**Your answer:**
The four panes are the Source pane, where I write and edit code; the Console pane, where R runs commands; the Environment pane, where I can see my datasets and objects; and the Files, Plots, Packages, and Help pane, where I can view files, graphs, packages, and help information.
...
**Your answer:** The dataset has _240_ observations.
...
**Your answer:**
NA means that the information is missing or not available for that observation.
...
**Your answer:**
North America appears to have the highest typical plastic waste per capita, while Oceania appears to have the lowest. North America also has the most variation because its values are more spread out and it has one unusually high value above 3.5 kg per person.
...
I chose an alpha level of 0.3 because it made the colors more transparent and made the overlapping distributions easier to see.
...
The differences is that color and fill are placed inside aes() because they change based on the continent in the data. The alpha value is placed inside geom_density() because it stays the same for every density curve.
...
Violin plots show the shape of the distribution and where most of the values are grouped together. Wider sections mean that more observations are in that area.


**What features are apparent in box plots but not in violin plots?**

Features box plots make it easier to see the median, quartiles, range, and possible outliers. That's why these values are not shown as clearly in the violin plots.
...
```{r plastic-waste-mismanaged, eval=TRUEE}
# Remember to change eval=FALSE to eval=TRUE!
ggplot(data = plastic_waste,
       mapping = aes(x = plastic_waste_per_cap,
...
There appears to be a mix of weak and positive relationship between plastic waste per capita and mismanaged plastic waste per capita, but the points are very spread out. Most countries have low values for both variables. There are also a few unusual points, including one country with very high plastic waste per capita.
---
...
There are no completely clear differences between the continents because most of the points overlap near the lower-left corner. However, North America also has the point with the highest plastic waste per capita.As well, some countries in Asia and Oceania have higher amounts of mismanaged plastic waste per capita. 
...
Both plastic waste per capita and coastal population appear to have a slightly stronger linear relationship. The relationship looks negative because countries with larger coastal populations generally have lower plastic waste per capita. However, neither graph shows a strong linear relationship, and both contain unusual points.
...
Plastic waste per capita generally rises as the coastal population proportion increases, but the relationship is not completely the same. The line rises until the coastal population proportion is around 1 and then begins to decrease slightly. The continents overlap a lot, so there are no completely clear differences. Many African countries have lower plastic waste per capita, while some North American and European countries have higher values.
